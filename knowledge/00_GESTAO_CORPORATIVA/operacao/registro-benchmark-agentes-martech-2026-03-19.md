...
Cada agente tem pacote completo em `apps/admin/agents/<slug>/`: AGENT.md, SOUL.md, USER.md, COMPANY.md, REDLINES.md, PERMISSIONS.md, VOICE.md, MEMORY.md, HEARTBEAT.md.

### 2. Nove skills novas

Criadas em `apps/admin/agents/skills/<slug>/SKILL.md`, com frontmatter (name, description, owner, persona, email, role, trigger_phrases) e seções: Objetivo, Quando usar, Input, Passos, Output, Critérios de revisão.
...
| **apps/admin/agents/grove_ceo.md** | Nova linha em "Delegação ao C-Suite": quando acionar benchmark_adventure (Ogilvy), benchmark_clientes (Cagan), benchmark_conteudo (Ogilvy). |
| **apps/admin/agents/skills/README.md** | Catálogo atualizado com as 9 skills (6 em CMO, 3 em CPO). |

---

## Referência para o C-Suite e Founder

- **Onde está:** Agentes em `apps/admin/agents/benchmark_adventure/`, `benchmark_clientes/`, `benchmark_conteudo/`. Skills em `apps/admin/agents/skills/benchmark-*`.
