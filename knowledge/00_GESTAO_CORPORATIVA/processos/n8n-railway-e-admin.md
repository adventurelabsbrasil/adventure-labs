# n8n — instalação e integração com o App Admin

> **Produção (mar/2026):** o n8n Adventure corre na **VPS Hostinger** via **Coolify** (ACORE Fase 4 — Comando Estelar). Exemplo de URL pública: `https://n8n.adventurelabs.com.br`. Configure `N8N_API_URL`, webhooks do Admin (`N8N_WEBHOOK_*`) e OAuth callbacks com esse domínio. **Railway deixou de ser o alvo** para esta instância.
>
> **Versionação:** fluxos do Comando Estelar em [`workflows/n8n/ce-n8n/README.md`](../../../../workflows/n8n/ce-n8n/README.md); restantes em [`workflows/n8n/README.md`](../../../../workflows/n8n/README.md).

Documento de referência para: (1) **hoje** — integrar o Admin com o n8n (webhooks, Public API, fluxos); (2) **arquivo** — passos que descrevem o **Railway** (template + Postgres) para quem ainda migra ou consulta histórico.

Ref.: conversas Grove (mar/2026), [github-project-e-integracao.md](../github-project-e-integracao.md) (padrão de integração).

---

## 1. Onde apontar URLs e tokens (VPS / Coolify)

| Uso | Variável / local | Valor típico |
|-----|------------------|--------------|
| Public API (import CLI, Admin) | `N8N_API_URL` | `https://n8n.adventurelabs.com.br` (base, sem `/webhook/...`) |
| Public API | `N8N_API_TOKEN` | Token em **n8n → Settings → API** |
| Webhooks disparados pelo Admin | `N8N_WEBHOOK_URL`, `N8N_WEBHOOK_CREATE_ISSUE`, etc. | `https://n8n.adventurelabs.com.br/webhook/...` (URL de **produção**, não `webhook-test`) |
| GitHub OAuth no n8n | Callback no GitHub OAuth App | `https://n8n.adventurelabs.com.br/rest/oauth2-credential/callback` |

Credenciais: Infisical / `.env.local` / Vercel — nunca no Git.

---

## 2. Por que n8n + Railway (arquivo / legado)

- **n8n**: automações por webhook (sem uso “constante”); alternativa open-source a Zapier/Make.
- **Railway** (histórico): deploy do template n8n + Postgres sem self-host manual; foi documentado até ~2026-Q1 antes da consolidação na VPS Hostinger + Coolify.
...
## 3. Passo a passo: Railway (n8n) — arquivo

### 3.1 Deploy do template
...
## 5. Configurar o App Admin

### 5.1 Variáveis de ambiente (webhook)
...
N8N_WEBHOOK_URL=https://n8n.adventurelabs.com.br/webhook/admin-evento
```

### 5.2 API do n8n (Public API) — token para o Admin chamar o n8n
...
2. **Salvar no Admin (ambiente local)**  
   ...
   N8N_API_URL=https://n8n.adventurelabs.com.br
...
## 6. Fluxo específico: Tarefa TI → criar issue no GitHub
...
### 6.2 Variável de ambiente no Admin
...
N8N_WEBHOOK_CREATE_ISSUE=https://n8n.adventurelabs.com.br/webhook/admin-criar-issue-github
```
...
### 6.3 Credencial GitHub no n8n (OAuth2 — conta gratuita)
...
2. Preencha:
   ...
   - **Homepage URL:** `https://n8n.adventurelabs.com.br` (ou a URL base do seu n8n).
   - **Authorization callback URL:** use **exatamente** o callback do n8n:
     ```
     https://n8n.adventurelabs.com.br/rest/oauth2-credential/callback
     ```
...
Use essa credencial no nó **GitHub** do workflow (seção 6.4). Se o repositório for de uma **organização**, garanta que o usuário que autorizou tem permissão de escrita no repo (ex. `adventurelabsbrasil/admin`).

---

### 6.4 Criar o workflow no n8n (passo a passo)
...
8. **Copiar a Production URL** do webhook (ex.: `https://n8n.adventurelabs.com.br/webhook/admin-criar-issue-github`) e colar em `N8N_WEBHOOK_CREATE_ISSUE` no Admin.

### 6.5 Uso no dia a dia
...
### 6.6 Verificação da cadeia (checklist)
...
### 6.7 Ajustes no canvas (passo a passo)
...
1. **URL no Admin:** Confirme que `N8N_WEBHOOK_CREATE_ISSUE` usa a **Production URL** do n8n: o path deve ser `/webhook/...` (ex.: `https://n8n.adventurelabs.com.br/webhook/admin-criar-issue-github`). Se estiver `/webhook-test/...`, está em modo teste — troque pela Production (copie do nó Webhook no n8n, com o workflow já ativado).
