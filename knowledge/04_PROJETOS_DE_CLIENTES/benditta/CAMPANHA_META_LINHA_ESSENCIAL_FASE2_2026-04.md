---
title: Benditta — Campanha Meta Ads | Linha Essencial (Fase 2 — Abr/2026)
domain: projetos_clientes
tags: [benditta, linha-essencial, meta-ads, instagram, formulario-nativo, fase2, rs]
updated: 2026-04-15
status: pronto_para_validacao_cliente
responsavel: igor
meta_account_id: act=763660518134498
business_id: "1159646178468576"
drive_criativos: "https://drive.google.com/open?id=128YsEU3UrbBfM4v-7IMnlOD0_AI969BO"
---

# Campanha Meta Ads — Linha Essencial (Fase 2)
**Período:** 17–30 de abril de 2026 · **Investimento:** R$ 600 · **Conta:** Benditta (act=763660518134498)

> Documento para validação interna (Adventure Labs) e aprovação da cliente Lisa (Benditta).
> Baseado nos aprendizados da Fase 1 (março/2026) e nos vídeos 3 e 4 da pasta de criativos no Drive.

---

## 1. Contexto e aprendizados da Fase 1

| Dado | Resultado |
|------|-----------|
| Período | 09–18/mar/2026 |
| Investimento | R$ 439,20 |
| Alcance | 31.797 pessoas |
| Conversas iniciadas (WhatsApp) | 10 |
| Custo por conversa | R$ 41,60 |
| Leads qualificados | ~2 de 10 |

**Principais ajustes para a Fase 2:**
- Destino do lead: WhatsApp → **Formulário Nativo do Meta** (filtragem por orçamento e projeto desde o anúncio)
- Foco de entrega: **Instagram** (feed, reels e stories)
- Região: **Região Metropolitana de Porto Alegre + Litoral do RS**
- Ticket mínimo de interesse: projetos acima de R$ 40–50 mil
- Qualificação explícita nos criativos: quem é e quem NÃO é o público

---

## 2. Objetivo da campanha

**Objetivo Meta Ads:** Geração de Cadastros (Lead Ads — Formulário Nativo Instagram)
**Meta de resultado:** 8–15 leads MQL em 15 dias / 10 dias úteis
**Métrica-guia:** Custo por lead qualificado (CPL alvo: abaixo de R$ 75)
**Qualificação mínima de MQL:** orçamento declarado ≥ R$ 30k + 2 ou mais ambientes + tem projeto (ou em andamento)
**Fluxo pós-form:** tela de agradecimento do Lead Form → botão "Falar no WhatsApp" → abre WhatsApp comercial Benditta

---

## 3. Estrutura técnica

| Nível | Decisão | Justificativa |
|-------|---------|---------------|
| **Conta** | act=763660518134498 | Conta de anúncios Benditta |
| **Campanha** | 1 campanha CBO | Orçamento fixo por ad set evita que um consuma o outro (verba equilibrada) |
| **Orçamento diário total** | R$ 50/dia | R$ 25/dia por ad set × 2 ad sets |
| **Duração** | 15 dias corridos / 10 dias úteis | Período: ~17–30 abr/2026 |
| **Investimento total estimado** | R$ 700–750 | R$ 50 × 14–15 dias |
| **Grupos de anúncio** | 2 conjuntos | Ad Set 1: Consumidor Final · Ad Set 2: Arquitetos |
| **Criativos** | Vídeo 3 + Vídeo 4 (Drive) | 1 vídeo por conjunto |
| **Destino** | Formulário Nativo → WhatsApp | Lead preenche form → tela de agradecimento abre WhatsApp comercial |
| **Plataforma** | Instagram (Feed + Reels + Stories) | Foco exclusivo Instagram nesta fase |
| **Região** | RMPA + Litoral RS | Público com capacidade de investimento |

---

## 4. Públicos (Ad Sets)

### Ad Set 1 — Consumidor Final (alto ticket)

**Objetivo:** Atrair pessoas com projeto de reforma/decoração completo e capacidade de investir R$ 40k+

**Segmentação:**
- Interesses: decoração de interiores, reforma residencial, arquitetura de interiores, compra de imóvel, imóveis de alto padrão, design de interiores
- Comportamentos: compradores de alto valor, proprietários de imóvel
- Filtros de renda/comportamento: sempre que disponível, usar sinais de renda alta (Meta: "rendas superiores", "compradores frequentes")
- Faixa etária: 30–58 anos
- Gênero: amplo
- Região: Região Metropolitana de Porto Alegre + Litoral do RS (Torres, Tramandaí, Capão da Canoa, Xangri-Lá)

**Exclusão recomendada:** não incluir interesses genéricos como "móveis" ou "decoração simples" que atraem baixo ticket

**Mensagem central:**
> "Você tem o projeto. A Benditta tem a execução.
> Marcenaria sob medida com padrão e clareza no investimento — sem improviso."

**CTA:** Cadastre-se
**Ângulo do vídeo (Vídeo 3 ou 4):** humaniza o processo, destaca qualidade, segurança na decisão, padrão da marca

---

### Ad Set 2 — Arquitetos e Designers de Interiores

**Objetivo:** Atrair arquitetos e designers da RMPA/litoral que buscam parceiro técnico de execução

**Segmentação:**
- Interesses: arquitetura de interiores, design de interiores, AutoCAD, SketchUp, BIM, Revit, projetos de construção
- Comportamentos: proprietários de pequenos negócios, profissionais autônomos
- Faixa etária: 25–50 anos
- Gênero: amplo
- Região: Região Metropolitana de Porto Alegre + Litoral do RS

**Mensagem central:**
> "Execução técnica de alto nível para seus projetos de marcenaria.
> Entregamos o que foi projetado — sem alterar o conceito, sem atrasar."

**CTA:** Saiba Mais
**Ângulo do vídeo (Vídeo 4 ou 3):** rigor técnico, fidelidade ao projeto, parceria profissional, prazo

> **Nota Igor:** confirmar qual dos 2 vídeos (3 ou 4) tem linguagem mais técnica/profissional
> e qual é mais emocional/residencial — isso define a alocação criativo × público.

---

## 5. Formulário Nativo MQL — campos validados

> **Fluxo completo:**
> Instagram → anúncio → formulário nativo (6 campos) → tela de agradecimento com botão "Falar no WhatsApp" → abre WhatsApp comercial Benditta
>
> **Lógica:** quem não tem projeto OU orçamento abaixo de R$ 15k ainda pode preencher — mas o time comercial
> prioriza atendimento para quem declarar R$ 30k+ e 2+ ambientes. Isso cria a fila de MQL sem bloquear volume.
>
> **Configuração técnica do Lead Form (Igor):**
> - Tipo de formulário: "Mais volume" (More Volume) — facilita preenchimento
> - Na aba "Tela de agradecimento": adicionar botão CTA tipo "Enviar mensagem" → inserir link `https://wa.me/[NUMERO_COMERCIAL_BENDITTA]` com mensagem pré-preenchida
> - Mensagem sugerida para o link: `Olá! Acabei de preencher o formulário da Linha Essencial e quero saber mais.`

**Campos do formulário:**

| # | Campo | Tipo | Opções |
|---|-------|------|--------|
| 1 | Nome completo | Texto | — |
| 2 | E-mail | E-mail | — |
| 3 | Telefone (WhatsApp) | Telefone | — |
| 4 | Você tem projeto? | Múltipla escolha | Sim / Ainda estou planejando / Não |
| 5 | Em quantos ambientes? | Múltipla escolha | 1 / 2 / 3 / 4 / 5 ou mais |
| 6 | Orçamento estimado | Múltipla escolha | Ver faixas abaixo |

**Faixas de orçamento (campo 6):**
- Abaixo de R$ 15.000
- R$ 15.000 – R$ 30.000
- R$ 30.000 – R$ 50.000
- R$ 50.000 – R$ 100.000
- R$ 100.000 – R$ 200.000
- Acima de R$ 200.000

**Critério MQL interno (não aparece para o lead):**
- Orçamento ≥ R$ 30k + ambientes ≥ 2 → MQL quente (atendimento prioritário Benditta)
- Orçamento ≥ R$ 30k + 1 ambiente OU projeto = "Não" → MQL morno (nutrir antes de abordar)
- Orçamento < R$ 15k → baixa prioridade comercial

> **Para Ad Set Arquitetos:** avaliar formulário simplificado:
> campo 4 substitui por "Quantos projetos de marcenaria você executa por ano?" (1–2 / 3–5 / 6 ou mais)

---

## 6. Copies dos anúncios

### Anúncio 1 — Consumidor Final

**Headline (até 40 caracteres):**
Sob medida, sem complicação

**Texto principal:**
Você não precisa entender de marcenaria para ter o melhor resultado.
A Linha Essencial Benditta foi criada pra quem quer qualidade, clareza no investimento e execução sem surpresas.
📍 Região Metropolitana de Porto Alegre e Litoral RS

**CTA:** Cadastre-se

---

### Anúncio 2 — Arquitetos

**Headline (até 40 caracteres):**
Execução que respeita o seu projeto

**Texto principal:**
Parceiro de marcenaria que entrega exatamente o que foi projetado — sem alterar conceito, sem improvisar no prazo.
Linha Essencial Benditta. Exclusivo para arquitetos e designers.
📍 Região Metropolitana de Porto Alegre e Litoral RS

**CTA:** Saiba Mais

---

## 7. Orçamento e cronograma

| Período | Ação | Verba diária | Acumulado |
|---------|------|-------------|-----------|
| 15–16/abr | Configuração: BM, criativos, formulário, lead form, WhatsApp link | — | — |
| **17–23/abr** (Semana 1 — 5 dias úteis) | Campanha no ar — fase de aprendizado algoritmo | R$ 50 | R$ 350 |
| **24–30/abr** (Semana 2 — 5 dias úteis) | Ajuste: analisar CPL, manter estrutura ou rebalancear copy | R$ 50 | R$ 350 |
| **Total** | | **R$ 50/dia** | **~R$ 700** |

**Distribuição por ad set:**
- Ad Set Consumidor Final: R$ 25/dia (Vídeo 3 ou 4)
- Ad Set Arquitetos: R$ 25/dia (Vídeo 4 ou 3)

**Regras de otimização:**
- Não pausar ad set antes de **3 dias completos** de dados
- Após dia 7: se 1 conjunto estiver com CPL 40%+ pior → revisar copy, não pausar ainda
- Se nenhum lead MQL nos primeiros 7 dias úteis → revisar copy e perguntas do formulário antes de qualquer pausa

---

## 8. KPIs e metas

| KPI | Referência Fase 1 | Meta Fase 2 |
|-----|------------------|-------------|
| Leads totais | 10 (via WhatsApp) | 8–15 (formulário) |
| Custo por lead (CPL) | R$ 41,60 | < R$ 75 |
| Taxa leads MQL (orç ≥ R$ 30k + 2+ amb) | ~20% | ≥ 40% |
| Custo por lead MQL | ~R$ 200+ | < R$ 150 |
| Taxa lead → contato respondido | n/a | > 70% (Benditta) |

**Janela mínima de leitura:** 5–7 dias após subida da campanha

---

## 9. Quinzena 1 de maio — planejamento (3 cenários)

**Cenário A — CPL < R$ 75 e ≥ 40% MQL:**
- Reinvestir R$ 600–800 com mesma estrutura
- Iniciar Google Ads — busca com termos de alta intenção (marcenaria sob medida Porto Alegre, marcenaria personalizada RS)
- Testar criativos estáticos (antes/depois) em paralelo como segundo criativo por ad set

**Cenário B — CPL aceitável mas taxa MQL < 40%:**
- Revisar copy de qualificação no formulário (campo de orçamento mais explícito)
- Ajustar segmentação do Ad Set Consumidor para reduzir baixa renda
- Manter apenas o ad set com melhor razão volume × qualidade

**Cenário C — CPL > R$ 100 ou volume < 5 leads:**
- Reunião de revisão rápida com a Lisa (15 min)
- Avaliar novos criativos (Lisa havia mencionado gravação pendente)
- Considerar LP própria como alternativa ao formulário nativo para maior controle de qualificação

---

## 10. Checklist pré-go-live

- [x] Conta de anúncios identificada: `act=763660518134498`
- [x] Criativos identificados: Vídeo 3 e Vídeo 4 (Drive `128YsEU3UrbBfM4v-7IMnlOD0_AI969BO`)
- [x] Região definida: RMPA + Litoral RS
- [x] Formulário MQL definido (6 campos com faixas de orçamento)
- [x] Públicos definidos (Consumidor Final + Arquitetos)
- [ ] **Confirmar qual vídeo vai para qual público** (3 → consumidor, 4 → arquiteto ou vice-versa)
- [ ] **Página do Facebook ativa e vinculada ao BM** (era bloqueio na Fase 1 — verificar)
- [ ] **Saldo disponível na conta** confirmado
- [ ] **Acesso Adventure Labs ao BM** da Benditta ativo
- [ ] **Validação da Lisa** nas copies e nas perguntas do formulário

---

## 11. O que NÃO mudamos (mantido da Fase 1)

- Posicionamento: Linha Essencial = **processo + padrão Benditta** (não preço)
- Dois pilares (consumidor final + arquiteto) em ad sets separados
- Narrativa: menos decisões técnicas, execução previsível, padrão Benditta
- Vídeo como formato principal (mais eficaz que estático nesta fase)
- CBO como modelo de otimização de verba
