...
-- Description: Sistema de rastreamento de tokens e custos de IA — gerido pelo agente Bill (Token Extractor)
-- Owner: Buffett (CFO) via Bill
...
CREATE TABLE IF NOT EXISTS public.adv_token_usage (
    ...
    collected_by TEXT DEFAULT 'bill',
    notes TEXT,
    created_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- Comments
-- ============================================================
COMMENT ON TABLE public.adv_ai_providers IS 'Inventario de provedores de IA (APIs, assinaturas, hibridos) gerido pelo Bill (Token Extractor)';
COMMENT ON TABLE public.adv_token_usage IS 'Snapshots periodicos de consumo de tokens e custos por provider, coletados pelo Bill';
COMMENT ON TABLE public.adv_token_alerts IS 'Historico de alertas de consumo, anomalias e oportunidades de economia gerados pelo Bill';
