...
### 5. **Em produção (Vercel): página “Configuração Supabase em falta”**

- **Causa:** o build de produção não recebeu `VITE_SUPABASE_URL` nem `VITE_SUPABASE_ANON_KEY` (variáveis não definidas para **Production** ou deploy antigo).
- **Solução:** Vercel → Project → Settings → Environment Variables → confira as duas chaves para **Production** → **Redeploy**. O app deixa de usar qualquer sessão fictícia em produção quando o cliente não existe.

### 6. **Abro /dashboard sem estar logado e vejo área vazia**

- **Causa:** falta de redirecionamento para `/login` em rotas protegidas.
- **Solução:** comportamento corrigido no código: sem sessão Supabase, rotas internas redirecionam para `/login` e, após login, voltam ao path pedido quando seguro (path relativo interno).

### 7. **Login aceito mas usuário sem permissões / tela em branco**
...
### 8. **Erro ao criar usuário via Configurações (email e senha)**
...
### 9. **Dependências / "Cannot find module"**
...
- [Qual projeto Vercel, tabela vs view `candidates`, teste `/apply/test`](./DEPLOY_VERCEL_E_CANDIDATOS.md)
