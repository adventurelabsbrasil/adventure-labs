...
/** Evita open-redirect via state.from (apenas paths internos). */
function safeReturnPath(p) {
  if (typeof p !== 'string' || !p.startsWith('/') || p.startsWith('//') || p.includes('://')) return '/dashboard';
  return p;
}
...
export default function LoginPage() {
  ...
  const from = safeReturnPath(location.state?.from) || '/dashboard';
  ...
  if (!supabase) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100 dark:from-gray-900 dark:to-gray-800 p-4">
        <div className="w-full max-w-md bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 border border-gray-200 dark:border-gray-700 text-center text-gray-600 dark:text-gray-400 text-sm">
          <p className="font-medium text-gray-900 dark:text-white mb-2">Supabase não configurado</p>
          <p>Defina <code className="text-xs bg-gray-100 dark:bg-gray-900 px-1 rounded">VITE_SUPABASE_URL</code> e{' '}
            <code className="text-xs bg-gray-100 dark:bg-gray-900 px-1 rounded">VITE_SUPABASE_ANON_KEY</code> em <code className="text-xs">.env.local</code> para testar o login localmente.</p>
        </div>
      </div>
    );
  }
