...
// Supabase
import { supabase, isSupabaseClientReady } from "./supabase";
...
export default function App() {
  ...
  const allowDevWithoutSupabase = import.meta.env.DEV && !isSupabaseClientReady;

  // Usuário efetivo: sessão ou DEV_USER só em dev quando Supabase não está configurado (nunca em produção)
  const effectiveUser = user ?? (allowDevWithoutSupabase ? DEV_USER : null);

  // Auth: sessão Supabase
  useEffect(() => {
    if (!supabase) {
      setAuthLoading(false);
      setUser(allowDevWithoutSupabase ? DEV_USER : null);
      return;
    }
  ...
  if (import.meta.env.PROD && !isSupabaseClientReady) {
