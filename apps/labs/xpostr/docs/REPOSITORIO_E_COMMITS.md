...
**Quando faz sentido:** um único lugar para Adventure Labs, deploy Vercel com **Root Directory** = `apps/labs/xpostr`.

**Commits só do Xpostr** (sem levar lixo do resto do mono):

```bash
git add apps/labs/xpostr/ knowledge/03_PROJETOS_INTERNOS/xpostr/
...
Copiar a pasta `apps/labs/xpostr/` para um repo novo e dar o primeiro commit lá.

**Caminho 2 — Subtree (mantém histórico da pasta)**  
Na raiz do monorepo:

```bash
git subtree split -P apps/labs/xpostr -b xpostr-split
...
Deploy na Vercel: em mono, configure **Root Directory** = `apps/labs/xpostr`; em repo dedicado, raiz do projeto.

**Deploy não atualiza?** No painel Vercel do projeto Xpostr confira **Root Directory = `apps/labs/xpostr`**. Se estiver em `apps/core/admin`, o push do Xpostr não altera esse deploy — crie outro projeto Vercel com root `apps/labs/xpostr` ou ajuste a root. **Redeploy manual:** Deployments → ⋮ → Redeploy.
