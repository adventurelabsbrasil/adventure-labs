...
**Barsi (Gestor de Patrimônio):** agente de apoio financeiro-patrimonial, **dual-mode** — owner Buffett (CFO) no modo Adventure, Founder direto no modo Personal. Modo Adventure (PJ): fotografia patrimonial semanal (saldos, investimentos, recebíveis, passivos, PL) em Supabase. Modo Personal (PF): snapshot mensal em `personal/barsi-patrimonio-pf/` (gitignored). Modo Consolidado: merge runtime PJ+PF (nunca persiste). Consulta Sueli (saldos), Chaves (acessos), Tostão (custos IA). Cron PJ: sexta 10:07 UTC. Tabelas: `adv_patrimony_accounts`, `adv_patrimony_snapshots`, `adv_patrimony_movements`. Docs: `knowledge/06_CONHECIMENTO/agents/barsi/`.
