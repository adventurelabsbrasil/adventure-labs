...
- **Um projeto Vercel por cliente**  
  Cada subdomínio é um projeto Vercel separado. O código é o mesmo (monorepo, `apps/admin`); as **Environment Variables** são as daquele cliente.
...
- Copie `clients/03_nomecliente/admin/.env.example` para `apps/admin/.env.local` e preencha com os valores reais (ou use um script que carrega o env da pasta do cliente). Rode o admin localmente; ele usará as credenciais daquele cliente.
...
| **Local** | `apps/admin/.env.local` — copiado do cliente ou carregado do `clients/XX_nome/admin/` para desenvolver aquele cliente. |
